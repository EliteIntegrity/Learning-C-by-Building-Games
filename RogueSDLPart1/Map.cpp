...
Map::Map(int width, int height)
    : width_(width)
    , height_(height)
    , cells_((size_t)width * (size_t)height)   // value-initialised (all Wall)
{
}

Tile& Map::at(int x, int y)
{
    return cells_[(size_t)y * width_ + x];
}
...
{
    for (Tile& t : cells_) t.visible = false;
}
...
void Map::render(SDL_Renderer* renderer, const GlyphCache& glyphs) const
{
    for (int y = 0; y < height_; ++y)
    {
        ...
        {
            const Tile& t = at(x, y);
            if (!t.explored) continue;  // unseen = invisible
